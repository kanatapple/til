...
参照はそれが指すリソースよりも長く生存することはできない  
Rust はこれが真であることを保証するために参照のスコープをチェックする
...
`y` は `x` が存在するスコープの中でだけ有効  
`x` がなくなった後にそれを指すことは不正になる

参照がそれの参照する変数より前に宣言された時も同じ問題が起こる  
これは同じスコープにあるリソースは宣言された順番と逆に解放されるから
...
上記の例では `y` が `x` より先に宣言されているので、`y` が `x` より長く生存することを意味する  
Rust ではこれは許されない